...
Conversación: mistral-small-latest | Visión: pixtral-12b-2409 | BD: Supabase
"""
...
# ── Verificación de comprobante (Mistral visión) ──
...
def verificar_comprobante(
    ...
    """
    Usa pixtral-12b-2409 para verificar si la imagen es un comprobante válido.
    ...
    prompt = f"""Analizá esta imagen y determiná si es un comprobante de pago válido.

Criterios que DEBE cumplir:
1. Ser un comprobante de transferencia bancaria (puede ser de cualquier banco o billetera: Mercado Pago, Brubank, Naranja X, BBVA, Galicia, etc.)
2. El destinatario (campo "Para" o similar) debe ser "{SENA_DESTINATARIO}" (puede tener variaciones menores de mayúsculas/minúsculas o acentos, o incluir segundo nombre como "Roberto Alejandro Santillan")
3. El monto debe ser ${monto_fmt} pesos argentinos (puede aparecer como ${monto_fmt}, {monto}, etc.)
4. La fecha del comprobante debe ser exactamente de hoy: {fecha_hoy}

IMPORTANTE: El campo "De" (quien envía el pago) puede ser cualquier persona. Solo verificás destinatario, monto y fecha.
Si encontrás algún identificador único del pago (puede llamarse "Número de operación", "Código de referencia", "ID de transacción", "Número de comprobante" u otro nombre similar), extraelo también.

Respondé ÚNICAMENTE con un JSON con este formato exacto, sin texto adicional:
{{"valido": true/false, "ilegible": true/false, "motivo": "explicación breve en español", "numero_operacion": "el número o null si no se encuentra"}}"""
    ...
    try:
        ...
        logger.error(f"Error verificando comprobante con Mistral visión: {e}")
        return {
            "valido": False,
            "ilegible": True,
            "motivo": "Error técnico al analizar la imagen",
