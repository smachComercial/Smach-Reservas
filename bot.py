...
def enviar_mensaje_whatsapp(wa_id: str, texto: str):
    ...
    # Normalizar números argentinos de usuarios: quitar el 9 después del 54 para el sandbox.
    # Se excluye el número del admin porque ya está almacenado en el formato correcto.
    if wa_id != ADMIN_WA_NUMBER and wa_id.startswith("549") and len(wa_id) == 13:
        wa_id = "54" + wa_id[3:]
...
- IMPORTANTE: Cuando el usuario pida ver sus reservas o consultar disponibilidad, emití la acción directamente sin avisar que "vas a buscar" ni pedir confirmación previa. No uses frases como "voy a buscar tus reservas", "un momento" o similares — simplemente ejecutá la acción.
...
- IMPORTANTE: Si el usuario pide hablar con una persona, un humano, el encargado, o dice que tiene una consulta especial, usa la accion derivar_humano inmediatamente. En tu respuesta de texto (antes del bloque ACCION) decile que puede contactar al administrador directamente por WhatsApp al +{ADMIN_WA_NUMBER}.
"""

def limpiar_frase_transicion(texto: str) -> str:
    """
    Cuando Mistral emite una acción, a veces incluye frases de transición como
    'Voy a buscar tus reservas, un momento...' que no deben enviarse al usuario
    porque el sistema ya ejecuta la acción en el mismo turno.
    Esta función retorna cadena vacía si el texto es solo una frase de ese tipo.
    """
    if not texto:
        return texto
    frases_transicion = [
    ...
    # Si el texto completo es una frase de transición (con posible puntuación al final),
    # lo descartamos. Usamos una heurística: si es corto y contiene alguna de esas frases.
    if len(texto) < 120:
        for frase in frases_transicion:
...
def manejar_mensaje_wa(wa_id: str, texto_usuario: str):
    ...
    try:
        ...
        if accion:
            ...
            # Sin acción: verificar si Mistral solo emitió una frase de transición
            # (ej: "Voy a buscar tus reservas, un momento...") sin ejecutar la acción.
            # En ese caso, forzamos una segunda llamada con el historial para que actúe.
            texto_limpio = limpiar_frase_transicion(texto_respuesta)
            if texto_limpio == "" and texto_respuesta.strip():
                # Era una frase de transición — forzar reintento con instrucción explícita
                logger.info(f"Frase de transicion detectada sin accion, reintentando: '{texto_respuesta[:80]}'")
                historial.append({"role": "assistant", "content": texto_respuesta})
                historial.append({
                    "role": "user",
                    "content": "<RESULTADO_SISTEMA>Ejecuta la accion correspondiente ahora mismo sin avisar que vas a hacerlo.</RESULTADO_SISTEMA>"
                })
                texto_final, accion_reintento = llamar_mistral(historial, hoy, telefono_conocido, nombre_conocido)
                if accion_reintento:
                    resultado_reintento = ejecutar_accion(accion_reintento)
                    logger.info(f"Reintento accion: {accion_reintento.get('tipo')} | Resultado: {resultado_reintento[:80]}")
                    historial.append({"role": "assistant", "content": texto_final})
                    historial.append({"role": "user", "content": f"<RESULTADO_SISTEMA>{resultado_reintento}</RESULTADO_SISTEMA>"})
                    texto_usuario_final, _ = llamar_mistral(historial, hoy, telefono_conocido, nombre_conocido)
                    historial.append({"role": "assistant", "content": texto_usuario_final})
                    sesion["historial"] = historial[-20:]
                    sesion_set(wa_id, sesion)
                    enviar_mensaje_whatsapp(wa_id, texto_usuario_final)
                else:
                    historial.append({"role": "assistant", "content": texto_final})
                    sesion["historial"] = historial[-20:]
                    sesion_set(wa_id, sesion)
