...
{{"valido": true/false, "ilegible": true/false, "motivo": "explicación breve en español de qué cumple o qué falta", "numero_operacion": "el número encontrado o null"}}"""
    ...
    try:
        ...
        logger.info(f"Pixtral resultado crudo: {resultado}")
        return resultado
    except json.JSONDecodeError as e:
        logger.error(f"Pixtral devolvió JSON inválido: {e} | Respuesta: {texto!r}")
        # JSON mal formado: no es culpa de la imagen, es un error técnico
        return {
            "valido": False,
            "ilegible": False,
        ...
        # Error de red, timeout, rate limit, etc. — no decirle al usuario que la imagen está borrosa
        return {
            "valido": False,
            "ilegible": False,
