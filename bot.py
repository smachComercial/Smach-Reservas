...
def ya_procesado(msg_id: str) -> bool:
    ...
    if len(_mensajes_procesados) >= _MAX_DEDUP_SIZE:
        # Limpiar la mitad más antigua (sets no tienen orden, pero evita crecimiento infinito)
...
def verificar_comprobante(
    ...
    # Formato largo en español para que Gemini reconozca ambas representaciones
    DIAS_ES = ["lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo"]
    MESES_ES = ["enero", "febrero", "marzo", "abril", "mayo", "junio",
                "julio", "agosto", "septiembre", "octubre", "noviembre", "diciembre"]
    ahora = ahora_arg()
    fecha_hoy_larga = f"{DIAS_ES[ahora.weekday()]} {ahora.day} de {MESES_ES[ahora.month - 1]} de {ahora.year}"
...
4. FECHA: Debe ser del día de hoy. Hoy es {fecha_hoy_larga} ({fecha_hoy}). El comprobante puede mostrar la fecha en cualquier formato: "{fecha_hoy}", "{fecha_hoy_larga}", o similar. Todos representan el mismo día — si la fecha del comprobante corresponde a hoy, es válida.
